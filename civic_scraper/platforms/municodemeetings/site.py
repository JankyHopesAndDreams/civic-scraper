...
class Site(base.Site):
    def __init__(self, base_url, cache=Cache(), parser_kls=Parser, place_name=None):
        ...
        #User agent because fuck MCM
        self.ua = { 'User-Agent': 'Mozilla 1.0' }
...
#        response = requests.get(search_url, params=params)
        ...
        print("Parsing addr is %s" % self.base_url)
        #MunicodeMeetings doesn't like user-agent-less requests because it's gay
        response = requests.get(self.base_url, headers=self.ua)

        print("Response() call worked")
#        pprint.pprint(response.text)
    ...
    def _build_asset_collection(self, metadata):
        ...
        for row in metadata:
            ...
            print("Testing to see if the requests.head is dying")
            headers = requests.head(url, allow_redirects=True, headers=self.ua).headers
            print("Nope, not what was dying")
            asset_args.update(
                {
